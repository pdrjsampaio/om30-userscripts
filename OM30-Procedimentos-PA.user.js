...
(() => {
  ...
  const aliases={
    ...
    procedimento:{
      ...
      'GESSO':'0301060100','TALA':'0301060100','IMOBILIZACAO':'0301060100'
    }
  };
  ...
  const favoritosUnidade={
    'UNIDADE TESTE GUARUJA':[
      ...
      {type:'procedimento',code:'0301060100',name:'ATENDIMENTO ORTOPÉDICO COM IMOBILIZAÇÃO PROVISÓRIA',query:'0301060100'},
  ...
  .oinfo{padding:8px 12px;background:#f3f7f9;border-bottom:1px solid #e1e8ec;font-size:11px;color:#5d7481}.tabs{display:flex;gap:6px;padding:9px 12px;border-bottom:1px solid #e5ebee}.tab{border:0;background:#edf3f6;color:#496473;padding:9px 12px;border-radius:9px;font-weight:700;cursor:pointer}.tab.on{background:#123f68;color:#fff}
  ...
  @media(max-width:650px){.grid{grid-template-columns:1fr}.mgrid{grid-template-columns:1fr}}
  `;
  ...
  <div class="oinfo"><b>${esc(UNIT)}</b> · Ocupação ${esc(OCC||'—')} · v0.1.0</div>
  <div class="tabs"><button class="tab on" data-t="exame">Exames</button><button class="tab" data-t="procedimento">Procedimentos / CIDs</button><button class="tab" data-t="medicamento">Medicamentos no local</button></div>
  <div class="obody">
    <div class="searchrow"><input class="search" placeholder="Ex.: radiografia, rx tórax, hemograma, hgt, pressão, gesso..."><button class="btn searchbtn">Pesquisar</button></div>
    <div class="hint">Aceita código SIGTAP, nome oficial e nomes populares. Exame/Procedimento já é incluído automaticamente no Saúde Simples.</div>
  ...
  let tipo='exame',timer;

  function status(t,k=''){E.st.textContent=t;E.st.className='status'+(k?' '+k:'')}
  function defs(){return (favoritosUnidade[UNITKEY]||[]).filter(x=>x.type===tipo)}
  ...
  async function usar(tipo,item){
    try{
      if(tipo==='medicamento'){composer(item);status('Medicamento selecionado. Preencha via e posologia.');return}
      status('Incluindo '+name(tipo,item)+'...');
      await incluirSimples(tipo,item);
      status(name(tipo,item)+' incluído. O destino da sala continua sendo definido pelo Saúde Simples.','ok');
  ...
  function renderFavs(){
    ...
    qa('.fav',E.lf).forEach(c=>{const it=fs[+c.dataset.i];q('.star',c).onclick=()=>{toggleFav(tipo,it);renderFavs()};q('.fuse',c).onclick=()=>usar(tipo,it)});
  }
  ...
  function renderTable(xs){
    ...
    const med=tipo==='medicamento';
    E.r.innerHTML='<div class="stitle"><span>'+(med?'Medicamentos disponíveis no local':'Tabela SIGTAP — resultados')+'</span><span class="muted">'+xs.length+' resultado(s)</span></div><div class="wrap"><table class="tbl"><thead><tr><th>★</th><th>'+(med?'Código':'Código SIGTAP')+'</th><th>'+(med?'Medicamento':'Procedimento')+'</th><th></th></tr></thead><tbody>'+xs.map((x,i)=>'<tr data-i="'+i+'"><td><button class="star '+(isFav(tipo,x)?'on':'')+'">'+(isFav(tipo,x)?'★':'☆')+'</button></td><td class="code">'+esc(code(tipo,x))+'</td><td><div class="nm">'+esc(name(tipo,x))+'</div></td><td><button class="use">'+(med?'Selecionar':'Usar + incluir')+'</button></td></tr>').join('')+'</tbody></table></div>';
    qa('tbody tr',E.r).forEach(tr=>{const it=xs[+tr.dataset.i];const st=q('.star',tr);st.onclick=()=>{const on=toggleFav(tipo,it);st.textContent=on?'★':'☆';st.classList.toggle('on',on);renderFavs()};q('.use',tr).onclick=()=>usar(tipo,it)});
  }
  ...
  async function pesquisar(forcado=null,grupo=false){
    ...
    const n=norm(termo);
    if(tipo==='exame'&&!grupo&&['RADIOGRAFIA','RX','RAIO X','RAIO-X'].includes(n)){renderRX();E.r.innerHTML='';status('Escolha a região da radiografia.');return}
    E.rx.innerHTML='';
    try{status('Pesquisando “'+traduz(tipo,termo)+'”...');const xs=await buscar(tipo,termo);renderTable(xs);status(xs.length+' resultado(s).')}catch(e){console.error(e);status(e.message||String(e),'err')}
  }

  qa('.tab',panel).forEach(b=>b.onclick=()=>{qa('.tab',panel).forEach(x=>x.classList.remove('on'));b.classList.add('on');tipo=b.dataset.t;E.s.value='';E.r.innerHTML='';E.rx.innerHTML='';E.mc.innerHTML='';renderFavs();status('Pronto.');E.s.focus()});
  ...
  renderFavs();
  status('Pronto.','ok');
  console.info('[OM30 PA] v0.1.0 carregada para',UNIT);
})();