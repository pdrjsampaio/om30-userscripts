...
(function () {
    ...
    // ============================================================
    // INTEGRAÇÃO GLPI - CRIAÇÃO DIRETA DE CHAMADOS
    // ============================================================
    ...
    function openGlpiLoginWindow(url) {
        ...
        try {
            ...
            glpiLoginWindowHandle =
                tab || null;

            if (typeof om30Log === 'function') {
                om30Log(
                    'glpi.open',
                    {
                        ok: true,
            ...
            console.warn(
                'OM30: GM_openInTab falhou; tentando window.open',
                error
            );
        }
        ...
        try {
            ...
            console.warn(
                'OM30: window.open do GLPI falhou',
                error
            );
        }
    ...
    function om30SanitizeLogValue(value, depth = 0) {
        ...
        if (typeof value === 'string') {
            if (/^data:image\//i.test(value)) {
                return `[data:image omitido · ${value.length} chars]`;
            }
            return value.length > 2500
                ? `${value.slice(0, 2500)}…`
                : value;
        }
        ...
        if (typeof value === 'object') {
            ...
            for (const [key, item] of Object.entries(value)) {
                if (
                    /cookie|authorization|csrf|token|password|passwd|session|secret/i.test(
                        key
                    )
                ) {
                    out[key] = '[REMOVIDO]';
    ...
    function om30DebugSnapshot() {
        let job = null;
        let queue = [];
        let result = null;

        try { job = readGlpiJob(); } catch {}
        try { queue = readGlpiQueue(); } catch {}
        try { result = readGlpiResult(); } catch {}
        ...
        return om30SanitizeLogValue({
            version: OM30_VERSION,
            url: location.href,
            chat: getChatName(),
            mode: getMode(),
            selected_count: selected.size,
            print_messages_count:
    ...
    function om30DownloadLogs() {
        try {
            ...
            a.href = url;
            a.download =
                `OM30-WhatsApp-GLPI-LOG-${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
            ...
            console.error(
                'OM30: falha baixando log',
                error
            );
            alert(
                'Não consegui gerar o log. Veja o Console do navegador.'
            );
        }
    }
    ...
    function dismissLastTicketForNewEvidence() {
        const last =
            getTicketHistory()[0];
        ...
        // Nunca esconde erro/login/progresso: só o cartão verde
        // do chamado anterior já concluído.
        if (
    ...
    function renderLastTicketMain() {
        ...
        // O X remove somente o cartão da tela.
        // O chamado continua normalmente no Histórico local e no GLPI.
        if (
            dismissedTicketId() ===
            String(last.ticket_id)
        ) {
            box.style.display = 'none';
            return;
        }
        ...
        if (close) {
            close.style.display = '';
            close.title = 'Remover este chamado da tela (continua no Histórico)';
            close.onclick = () =>
                dismissTicketResult(
                    last.ticket_id
                );
    ...
    function setCreateButtonFeedback(
        ...
        // Mantém bloqueado até chegar sucesso/erro/login.
        // Assim preservamos a ficha sem permitir um clique duplicado.
    }

    function releaseCreateButton(text = 'CRIAR CHAMADO') {
        const button =
            document.getElementById(
                'om30-validate'
            );
    ...
    function syncGlpiResult() {
        ...
        if (result?.kind === 'background-error' || result?.status === 'error') {
            ...
            if (view) {
                // renderSilentProgress() desabilita este botão.
                // Em erro ele precisa ser explicitamente reativado.
                view.disabled = false;
            ...
            if (close) {
                close.onclick = () => {
                    ...
                    renderLastTicketMain();
                    releaseCreateButton('TENTAR NOVAMENTE');
        ...
        if (result?.kind === 'login-required' || result?.status === 'login-required') {
            ...
            if (view) {
                // O progresso anterior deixou o botão disabled.
                view.disabled = false;
            ...
            if (close) {
                close.onclick = () => {
                    ...
                    box.classList.remove('om30-result-login');
                    renderLastTicketMain();
                    releaseCreateButton('CRIAR CHAMADO');
                };
            }
        ...
        // Chamado confirmado: só agora começamos uma ficha nova.
    ...
    function messageMedia(content, bubble) {
        const root =
            bubble ||
            content;
        ...
        for (
            ...
            const likelyMessageMedia =
                /^blob:/i.test(src) ||
                /^data:image\//i.test(src) ||
                /whatsapp|mmg|pps\.whatsapp/i.test(src);
    ...
    function toggleMessage(info) {
        dismissLastTicketForNewEvidence();
        ...
        event => {
            ...
            if (
                event.target.closest(
                    `#${ID.panel}, #${ID.launcher}, #${ID.camera}, #${ID.reader}, #${ID.selectionBar}`
                )
            ) {
                return;
            }
    ...
    function detectInitialDate() {
        ...
        for (
            ...
            return {
                ...
                score: 100,
                reason:
                    'primeira mensagem da evidência'
            };
        }
    ...
    function messagesInsideCrop(crop) {
        ...
        for (
            ...
            const r =
                msg.element.getBoundingClientRect();
    ...
    const OPERATIONS = [
        [567, "Guarujá"], [587, "Arujá"], [591, "Caieiras"], [28, "Cotia"],
        [610, "Ferraz"], [586, "Hortolândia"], [585, "Ibiúna"], [540, "Itapevi"],
        [273, "Jahu"], [588, "Jales"], [590, "Mauá"], [592, "Mongaguá"],
        [589, "São Sebastião"]
    ];
    ...
    function createUI() {
        ...
        panel.innerHTML = `
            ...
            <div id="${ID.settingsMenu}" class="om30-settings-menu">
                ...
                <button type="button" id="om30-settings-download-log">BAIXAR LOG</button>
                <button type="button" id="om30-settings-clear-log">LIMPAR LOG</button>
                <div class="om30-settings-line">
                    <span>Eventos no log</span>
                    <b id="om30-settings-log-count">0</b>
                </div>
            </div>
        ...
        const operation = panel.querySelector('#om30-operation');
        const opSaved = localStorage.getItem(KEY.operation);
        if (opSaved) operation.value = opSaved;
        operation.onchange = () => localStorage.setItem(KEY.operation, operation.value);
        ...
        panel.querySelector('#om30-settings-download-log').onclick = () => {
            om30DownloadLogs();
            renderSettingsMenu();
        };
        ...
        panel.querySelector('#om30-settings-clear-log').onclick = () => {
            if (
                confirm(
                    'Limpar o log de diagnóstico OM30 deste navegador?'
                )
            ) {
                om30ClearLogs();
            }
    ...
    // Localizações obtidas do próprio Select2 do GLPI de Guarujá.
    ...
    // IDs confirmados diretamente no Select2 do GLPI.
    ...
    async function mediaSourceToDataUrl(
        ...
        if (!response.ok) {
            throw new Error(
                `Imagem do WhatsApp respondeu HTTP ${response.status}.`
            );
        }
        ...
        if (
            ...
            throw new Error(
                `Conteúdo selecionado não é imagem (${blob.type || 'tipo desconhecido'}).`
            );
        }
    ...
    async function dataUrlToLoadedImage(
        dataUrl
    ) {
            ...
            (resolve, reject) => {
                ...
                img.onerror =
                    () =>
                        reject(
                            new Error(
                                'Não consegui carregar uma das imagens selecionadas.'
                            )
                        );
    ...
    async function validateTicket() {
        ...
        try {
            ...
            // A fonte oficial das imagens na v0.9.14 é evidenceImages.
            const printDataUrl =
            ...
            // NÃO limpa a ficha antes da confirmação.
            // Se categoria/unidade/login falhar, o usuário não perde
            // print, mensagens, datas ou campos preenchidos.
            scrollPanelToProcess('smooth');
    ...
    function runOm30IntegrationSelfCheck() {
        ...
        const confirmed =
            SILENT_CONFIRMED_LOCATIONS['567'] ||
            [];
        ...
        if (missingUnits.length) {
            errors.push(
                `Unidades sem ID confirmado: ${missingUnits.join(', ')}`
            );
        }
        ...
        for (
            ...
            if (
                ...
                errors.push(
                    `${name}: esperado ${expectedId}, recebido ${item?.id || 'ausente'}`
                );
            }
        }
